  ...
  if (status != OK) error.print(status);
      ...
      while ((status = scan1->scanNext(rec2Rid)) != FILEEOF)
	{
	  ...
	  if ((i % 2) != 0)
		...
		{
		  cout << "err0r status return from deleteRecord242" << endl;
		  error.print(status);
		}
	    }
	  i++;
  ...
  if (status != OK) error.print(status);
  else 
    {
	  ...
	  if ( i > 0 ) // don't delete the first one
	      ...
	      if ((status != OK) && ( status != NORECORDS))
                {
		  cout << "err0r status return from deleteRecord274" << endl;
		  error.print(status);
                }
            }
	  i++;
	  ...
	  if ((status != OK) && ( status != NORECORDS))
            {
	      cout << "err0r status return from deleteRecord297" << endl;
	      error.print(status);
            }
	  i++;
        }
  ...
  if (status != OK) error.print(status);
      ...
      while ((status = scan1->scanNext(rec2Rid)) != FILEEOF)
	{
	  ...
	  if ((i > 1000) && ( i <= 2000))
	      ...
	      if ((status != OK) && ( status != NORECORDS))
                {
		  cout << "err0r status return from deleteRecord508" << endl;
		  error.print(status);
                }
	      else numDeletes++;
            }
